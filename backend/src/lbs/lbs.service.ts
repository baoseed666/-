...
export class LbsService {
  ...
  constructor(
    @InjectRepository(LbsRequest) private readonly repo: Repository<LbsRequest>,
  ) {}
  ...
  async findMatch(
    ...
    lng: number,
    tags: string[],
  ): Promise<{ userId: string; requesterSocketId: string } | null> {
    ...
    for (const c of candidates) {
      ...
      if (dist <= MATCH_RADIUS_KM && hasCommonTag) {
        return {
          userId: c.user.id,
          requesterSocketId: this.socketMap.get(c.user.id) ?? '',
        };
      }
    }
    return null;
  ...
  async confirmMatch(userId1: string, userId2: string) {
    await this.repo.update(
      { user: { id: userId1 } as any },
      { status: LbsStatus.MATCHED, matchedUserId: userId2 },
    );
    await this.repo.update(
      { user: { id: userId2 } as any },
      { status: LbsStatus.MATCHED, matchedUserId: userId1 },
    );
  }
