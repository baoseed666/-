...
### 后端
- **非 AI 接口：** Jest 正常结果 + 预期异常测试
- **AI 接口：** 复杂场景测试（AI 决策能力 + 工具执行能力）

### 前端
- **Playwright E2E：** 桌面端 + 移动端双设备
- **覆盖范围：** 完整业务链路（逐按钮/逐输入/逐跳转模拟真实用户）
- **正常场景：** 完整流程走通
- **边界场景：** 预期内异常正确拦截

### 代码质量原则
- 任何修改必须从根源重构融合，严禁补丁式修复
- 结果代码最简但完整，不留多余层
- 关键模块完成后调用 `superpowers:code-reviewer` 独立审查
